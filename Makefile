...
all: $(plot_overview) $(plot_overview_regressor) $(plot_roc) $(plot_hists) $(gamma_dl3)
...
$(proton_test) $(proton_train): $(proton_output) | $(build_dir)
	klaas_split_data $(proton_output) $(build_dir)/protons -n test -f 0.3 -n train -f 0.7  -d array_event_id

$(gamma_test) $(gamma_train): $(gamma_output) | $(build_dir)
	klaas_split_data $(gamma_output) $(build_dir)/gammas -n test -f 0.3 -n train -f 0.7  -d array_event_id
