...
def calculate_sensitivity(
    ...
    # print(n_on, n_off, sens)
    if np.isnan(sens.value):
        print('NANSDAND')
        print(n_on, n_off)
    return sens
...
def create_sensitivity_matrix(
    ...
    if num_threads > 1:
        ...
        sensitivity = [find_best_sensitivity_in_bin(gammas[gammas.energy_bin == b], protons[protons.energy_bin == b], b) for b in tqdm(gammas.energy_bin.cat.categories)]
    ...
    return sensitivity, bin_edges


def find_best_sensitivity_in_bin(g, p, energy_bin):
    ...
    cuts = res[0]
    return calculate_sensitivity(g, p,  min_energy=energy_bin.left, max_energy=energy_bin.right, gamma_prediction_mean=cuts[0], signal_region=cuts[1])
